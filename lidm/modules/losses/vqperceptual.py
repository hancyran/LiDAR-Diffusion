...
class VQGeometryWithDiscriminator(nn.Module):
    def __init__(self, disc_start, codebook_weight=1.0, pixelloss_weight=1.0,
                 ...
                 geo_factor=1.0, curve_length=4, bev_only=False, dataset_config=dict()):
        super().__init__()
        ...
        print(f"VQGeometryWithDiscriminator running with {disc_loss} loss.")
        self.disc_factor = disc_factor
        self.discriminator_weight = disc_weight
        self.disc_conditional = disc_conditional
        self.n_classes = n_classes

        self.geometry_converter = GeoConverter(curve_length, bev_only, dataset_config)
    ...
    def forward(self, codebook_loss, inputs, reconstructions, optimizer_idx,
        ...
        input_coord = self.geometry_converter(inputs.contiguous())
        ...
        geo_rec_loss = self.geo_loss(input_coord, rec_coord) * self.geo_factor

        rec_loss = (pixel_rec_loss + geo_rec_loss + mask_rec_loss) / self.rec_scale
        ...
        if optimizer_idx == 0:
            if cond is None:
                ...
                logits_fake = self.discriminator(torch.cat((reconstructions.contiguous(), rec_coord.contiguous()), dim=1))
            else:
                assert self.disc_conditional
                logits_fake = self.discriminator(torch.cat((reconstructions.contiguous(), rec_coord.contiguous(), cond), dim=1))
        ...
        if optimizer_idx == 1:
            ...
            if self.mask_factor > 0 and masks is not None:
                inputs = torch.cat((inputs, masks.contiguous().detach()), dim=1)

            inputs = torch.cat((inputs, input_coord.contiguous().detach()), dim=1)
            reconstructions = torch.cat((reconstructions, rec_coord.contiguous().detach()), dim=1)
